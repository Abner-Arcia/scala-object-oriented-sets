...
class TweetSetSuite extends munit.FunSuite:
  ...
  test("filter: four tweets of length 6 on set5") {
    new TestSets:
      assertEquals(size(set5.filter(tw => tw.text.length == 6)), 4)
  }
  ...
  test("mostRetweeted: 20 with set5") {
    new TestSets:
      assertEquals(set5.mostRetweeted.retweets, 20)
  }

  test("remove: c with set5") {
    new TestSets:
      assertEquals(size(set5.remove(c)), 3)
  }
